...
int main(int argc, char* argv[])
{
  ...
  for(size_t platIndex = 0; platIndex < all_platforms.size(); ++platIndex)
  {
    const cl::Platform& platform = all_platforms[platIndex];

    std::cout << "[" << platIndex << "]: " << platform.getInfo<CL_PLATFORM_NAME>() << std::endl;
    ...
    {
      const cl::Device& device = devices[devIndex];

      std::cout << "\t[" << devIndex << "]: " << device.getInfo<CL_DEVICE_NAME>() << std::endl;
    }
  }
  ...
  cl::Platform default_platform = all_platforms[platformIndex];
  ...
  cl::Device default_device = all_devices[deviceIndex];
  std::cout << "Using device: " << default_device.getInfo<CL_DEVICE_NAME>() << "\n";

  auto contextCallbackFunc = [](const char* errinfo, const void* privateInfo, size_t cb, void* userData)
  {
    std::cout << "[OpenCL Context Callback]: " << errinfo << std::endl;
  };

  cl_context_properties* contextProperties = nullptr;
  void* userData = nullptr;
  cl_int err = CL_SUCCESS;
  cl::Context context({ default_device }, contextProperties, contextCallbackFunc, userData, &err);
